...
class AuthService {
  async login(username: string, password: string): Promise<AuthResponse> {
    console.log('AuthService: Fazendo requisição de login para:', `${API_URL}/auth/login`)
    try {
  ...
  async register(name: string, username: string, password: string): Promise<AuthResponse> {
    console.log('AuthService: Fazendo requisição de registro para:', `${API_URL}/auth/register`)
    try {
      ...
      console.error('AuthService: Erro no registro:', error.response?.data || error.message)
      throw error // Re-throw para que o componente possa capturar
    }
  }
}
